...
- **Runtime status** persists in `office_agent_runtime` (PK `agent_id`). On restart a `paused` agent stays `paused`, `pause_reason` (e.g. `"budget"`) is preserved, and `last_run_finished_at` is retained so the cooldown guard works across restarts. The heartbeat cooldown gate (`heartbeatAgentRuntime.AllowFire` in `internal/backendapp/cron.go`) reads this `office_agent_runtime.last_run_finished_at` column together with `agent_profiles.cooldown_sec` to decide whether enough time has elapsed since the agent's last run. `agent_profiles` carries its own same-named `last_run_finished_at` column, inherited from the shared kanban agent-settings table — no Office write path stamps it, so it is not the cooldown source of truth and must not be read for gating. Every terminal run for an agent — completed, stopped, taskless-completed, or failed — stamps `office_agent_runtime.last_run_finished_at` via `Service.stampRunFinished`, so `cooldown_sec` paces the next heartbeat-driven fire uniformly regardless of outcome; see [Office Agent Runtime — Error Handling Contract](../requirements/runtime.md) for the failure-path detail.
