...
- **GIVEN** an `agent_paused_after_failures` inbox entry, **WHEN** the user clicks Mark fixed and the agent's `pause_reason` still matches the value that the service read at the start of the request, **THEN** the compare-and-set clear succeeds, `consecutive_failures` resets to zero, the entry is dismissed, and a wakeup with reason `manual_resume_after_failure` is queued for every (task, agent) listed on the entry (`AC-OFFICE-RUNTIME-001.9`, `AC-OFFICE-RUNTIME-001.10`).
- **GIVEN** an `agent_paused_after_failures` inbox entry, **WHEN** a second auto-pause overwrites the agent's `pause_reason` after the service read and before the conditional write, **THEN** the compare-and-set clear is refused, the call aborts instead of adopting the newer reason, `consecutive_failures` is not reset, no wakeup is queued, and the inbox entry is not dismissed so it stays actionable for the still-paused agent (`AC-OFFICE-RUNTIME-001.9`, `AC-OFFICE-RUNTIME-001.10`).
