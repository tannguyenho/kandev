...
func TestStore_IssueWatch_Update(t *testing.T) {
	...
	if got.Filter.Query != "deadlock" || got.Enabled || got.PollIntervalSeconds != 60 {
...
func TestStore_IssueWatch_DeleteCascadesDedupRows(t *testing.T) {
	...
	if _, err := store.ReserveIssueWatchTask(ctx, w.ID, "PROJ-1", "https://sentry.io/issues/PROJ-1"); err != nil {
	...
	// Re-creating with the same ID and reserving the same short_id should
	// succeed without UNIQUE collision — proves cascade ran.
	w2 := newTestIssueWatch("ws-1")
	w2.ID = w.ID
	if err := store.CreateIssueWatch(ctx, w2); err != nil {
		t.Fatalf("recreate watch: %v", err)
	}
	ok, err := store.ReserveIssueWatchTask(ctx, w2.ID, "PROJ-1", "https://sentry.io/issues/PROJ-1")
	if err != nil {
...
func TestStore_IssueWatchTask_ReserveDedup(t *testing.T) {
	...
	first, err := store.ReserveIssueWatchTask(ctx, w.ID, "PROJ-7", "https://sentry.io/issues/PROJ-7")
	if err != nil {
	...
	second, err := store.ReserveIssueWatchTask(ctx, w.ID, "PROJ-7", "https://sentry.io/issues/PROJ-7")
	if err != nil {
	...
	seen, err := store.ListSeenIssueShortIDs(ctx, w.ID)
	if err != nil {
...
func TestStore_IssueWatchTask_AssignTaskID(t *testing.T) {
	...
	if err := store.AssignIssueWatchTaskID(ctx, w.ID, "PROJ-NOPE", "task-zzz"); err == nil {
...
func TestStore_IssueWatchTask_Release(t *testing.T) {
	...
	again, err := store.ReserveIssueWatchTask(ctx, w.ID, "PROJ-1", "https://sentry.io/issues/PROJ-1")
	if err != nil {
		t.Fatalf("re-reserve: %v", err)
	}
	if !again {
...
func TestStore_IssueWatchTask_ListTaskIDsAndReset(t *testing.T) {
	...
	for _, k := range []string{"PROJ-1", "PROJ-2", "PROJ-3"} {
		if _, err := store.ReserveIssueWatchTask(ctx, w.ID, k, "https://sentry.io/issues/"+k); err != nil {
