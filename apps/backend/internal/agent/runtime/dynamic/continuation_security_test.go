...
func TestSanitizedTailRedactsAnchorSeparatedFromValueAtWindowCut(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if strings.Contains(routingerr.Sanitize(tc.anchor+tc.value), tc.value) {
				t.Fatalf("precondition failed: routingerr.Sanitize does not redact %q", tc.anchor+tc.value)
			}
...
func TestSanitizedTailDropsValueAfterAnchorBeyondLookback(t *testing.T) {
	...
	if strings.Contains(routingerr.Sanitize(anchor+separator+credential), credential) {
